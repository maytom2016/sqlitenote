...
def updatetc():# 更新表内容
   ...
   if (label2.get() != '0') and (len(text.get("1.0","end-1c"))<20000):
       if re:
           classifyA = players.get()
           # "新分类"
       else:
           classifyA = getclassify()
       data = (time.strftime('%Y-%m-%d %H:%M:%S' , time.localtime(time.time())),classifyA,text1.get("1.0","end-1c"),text.get("1.0","end-1c"),label2.get())
       conn = sqlite3.connect(DB_FILE_PATH)
       cu = conn.cursor()
       cu.execute(update_sql,data)
       conn.commit()
       cu.close()
       item=label2.get()+","+title
       #item='awa'
       lb.insert(END,'')#防抖动
       lb.delete(LID,LID)
       lb.insert(LID,item)
       lb.delete(END,END)#防抖动


       if re:
         initcomb()
         players.current(getcomboxid(classifyA))
         combbox_click()

   elif(label2.get() =='0') and (len(title) >1):#此条件为了防止空标题
       newnote();
   elif(len(title) <=1):
       messagebox.showinfo('提示', '空标题不允许保存！')
   else:
...
def newnotefc(title,content):
    ...
    if isnewclassify():
        initcomb()
        players.current(getcomboxid(classify1))
        combbox_click()
    else:
...
sc = Scrollbar(mygui,orient=tk.VERTICAL,width=15)

lb = Listbox(mygui, listvariable="qwe",width=15,height=31,yscrollcommand=sc.set)  # 实例化一个Listbox/listvariable指定列表内容
#lb.bind('<Double-Button-1>',loadcontent)
lb.bind('<Double-Button-1>',loadcontent)
lb.place(x=0,y=25)


sc.place(x=110,y=22,height=570)
